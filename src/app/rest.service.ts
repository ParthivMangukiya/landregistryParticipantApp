...
//  signUp(data) {
 ...
 // }
  ...
  signUp(data) {
     ...
     return this.httpClient.post('http://localhost:3001/api/PrivateIndividual', privateIndividual).toPromise()
       .then(() => {
         const identity = {
           participant: 'org.svnit.comps.PrivateIndividual#' + data.id,
           userID: data.id,
           options: {}
         };
        return this.httpClient.post('http://localhost:3001/api/system/identities/issue', identity, {responseType: 'blob'}).toPromise();
